...
#from merge import mer
import os
import argparse

#import importlib
def gmail_read(username):
# Creating a storage.JSON file with authentication details
	SCOPES = 'https://www.googleapis.com/auth/gmail.modify' # we are using modify and not readonly, as we will be marking the messages Read
	module_dir = os.path.dirname(__file__)
	store = file.Storage(os.path.join(module_dir, f'jsonfile/{username}.json'))
	flags = tools.argparser.parse_args(args=[])
	creds = store.get()
	if not creds or creds.invalid:
	    flow = client.flow_from_clientsecrets(os.path.join(module_dir, 'client_secret.json'), SCOPES)
	...
	with open(os.path.join(module_dir,f'csvfile/{username}.csv'), 'w', encoding='utf-8', newline = '') as csvfile:
...
def main():
 ...
 run(username)
 mer(username)
	#importlib.import_module("auto.py")
